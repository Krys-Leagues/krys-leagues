create or replace function public.generate_match_next_season_proposal(
...
begin
  ...
  if v_user_id is null then
    raise exception 'Authentication is required to generate a Match next-season proposal' using errcode = '42501';
  end if;
  ...
  select * into v_scorecard from public.match_final_scorecards
  where id = p_final_scorecard_id for share;
  if not found or v_scorecard.status <> 'approved' then
    raise exception 'An approved Match Final Scorecard is required';
  end if;

  select * into v_source_season from public.seasons where id = v_scorecard.season_id for share;
  if not found or lower(btrim(v_source_season.league_type)) is distinct from 'match' then
    raise exception 'The source season is not a valid Match season';
  end if;
  select * into v_target_season from public.seasons where id = p_target_season_id for share;
  if not found or lower(btrim(v_target_season.league_type)) is distinct from 'match' then
    raise exception 'The target season is not a valid Match season';
  end if;
  if v_target_season.season_number <> v_source_season.season_number + 1 then
    raise exception 'Target Match season number must equal source season number + 1';
  end if;
  ...
  left join public.match_final_scorecard_player_decisions as decision
  ...
  where entry.scorecard_id = p_final_scorecard_id and not exists (
    select 1 from public.match_final_scorecard_player_decisions as decision
  ...
  if exists (select 1 from unnest(p_new_player_ids) as item(player_id)
    join public.match_final_scorecard_entries as entry on entry.player_id = item.player_id
  ...
  select count(*)::integer into v_draft_count from public.match_roster_versions
  ...
  if exists (select 1 from public.match_roster_versions where season_id = p_target_season_id
    and status in ('approved', 'locked')) then
    raise exception 'An approved or locked target roster cannot be replaced';
  end if;

  select * into v_roster from public.match_roster_versions
  ...
  if found then
    ...
    if v_roster.source_final_scorecard_id is null and exists (
      select 1 from public.match_division_roster_slots as existing_slot
    ...
    update public.match_roster_versions set source_final_scorecard_id = p_final_scorecard_id,
      division_count = p_target_division_count, updated_at = now() where id = v_roster.id returning * into v_roster;
  else
    insert into public.match_roster_versions (season_id, division_count, status, source_final_scorecard_id)
    values (p_target_season_id, p_target_division_count, 'draft', p_final_scorecard_id)
    returning * into v_roster;
  end if;

  create temporary table if not exists pg_temp.match_transition_work (
  ...
  truncate pg_temp.match_transition_work;

  insert into pg_temp.match_transition_work (
  ...
  select entry.player_id, entry.player_screen_name, entry.division_number,
    ...
    ((entry.completed_game_count = 0 and entry.division_number < v_source_division_count)
      or entry.division_rank = (
        ...
        from public.match_final_scorecard_entries as all_entry
  ...
  from public.match_final_scorecard_entries as entry
  join public.match_final_scorecard_player_decisions as decision
    on decision.final_scorecard_id = entry.scorecard_id and decision.player_id = entry.player_id
  where entry.scorecard_id = p_final_scorecard_id and decision.decision = 'returning';

  if exists (select 1 from pg_temp.match_transition_work
  ...
  update pg_temp.match_transition_work set target_division = source_division + 1,
  ...
  for v_division in 1..greatest(v_source_division_count - 1, 0) loop
    ...
    where entry.scorecard_id = p_final_scorecard_id and entry.division_number = v_division
      and (decision.decision = 'not_returning' or exists (
        select 1 from pg_temp.match_transition_work as work
    ...
    loop
      update pg_temp.match_transition_work as work set target_division = v_division,
        movement_reason = 'Promoted' where work.player_id = v_player.player_id;
    end loop;
  end loop;

  if exists (select target_division from pg_temp.match_transition_work where target_division is not null
    group by target_division having count(*) > 4) then
    raise exception 'Mandatory movement exceeds a target division capacity';
  end if;

  for v_player in select * from pg_temp.match_transition_work where target_division is null
  ...
  loop
    ...
    loop
      if v_division between 1 and p_target_division_count and (
        select count(*) from pg_temp.match_transition_work where target_division = v_division
    ...
    update pg_temp.match_transition_work as work set target_division = v_target,
  ...
  create temporary table if not exists pg_temp.match_transition_new_players (
    player_id uuid primary key, player_screen_name text not null, selection_order integer not null,
    target_division integer
  ) on commit drop;
  truncate pg_temp.match_transition_new_players;
  insert into pg_temp.match_transition_new_players (player_id, player_screen_name, selection_order)
  ...
  for v_player in select * from pg_temp.match_transition_new_players order by selection_order loop
    v_target := null;
    if p_target_division_count > v_source_division_count then
      for v_division in v_source_division_count + 1..p_target_division_count loop
        if (select count(*) from pg_temp.match_transition_work where target_division = v_division)
          + (select count(*) from pg_temp.match_transition_new_players where target_division = v_division) < 4
    ...
    update pg_temp.match_transition_new_players as new_player
    set target_division = v_target
    where new_player.player_id = v_player.player_id;
  end loop;

  delete from public.match_division_roster_slots as roster_slot
  where roster_slot.roster_version_id = v_roster.id;
  insert into public.match_division_roster_slots (
  ...
  for v_division in 1..p_target_division_count loop
    ...
    loop
      select min(roster_slot.slot_number) into v_slot from public.match_division_roster_slots as roster_slot
    ...
    for v_player in select * from pg_temp.match_transition_new_players
  ...
  left join pg_temp.match_transition_work as work on work.player_id = slot.player_id
  left join pg_temp.match_transition_new_players as new_player on new_player.player_id = slot.player_id
  where slot.roster_version_id = v_roster.id
  order by slot.division_number, slot.slot_number;
end;
$function$;

revoke all on function public.generate_match_next_season_proposal(uuid, uuid, integer, uuid[]) from public;
revoke all on function public.generate_match_next_season_proposal(uuid, uuid, integer, uuid[]) from anon;
revoke all on function public.generate_match_next_season_proposal(uuid, uuid, integer, uuid[]) from authenticated;
grant execute on function public.generate_match_next_season_proposal(uuid, uuid, integer, uuid[]) to authenticated;
