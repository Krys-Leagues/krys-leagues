...
begin
  ...
  if found then
    update public.pyp_managed_results result set course1_name=btrim(p_course1_name),course1_home_hw=p_course1_home_hw,course1_away_hw=p_course1_away_hw,
      course2_name=btrim(p_course2_name),course2_home_hw=p_course2_home_hw,course2_away_hw=p_course2_away_hw,winner_player_id=v_winner,is_draw=v_draw,result_id=v_public_result.id
    ...
    insert into public.pyp_managed_results(result_id,schedule_id,season_id,roster_version_id,division_number,game_number,home_player_id,away_player_id,
      home_player_screen_name,away_player_screen_name,course1_name,course1_home_hw,course1_away_hw,course2_name,course2_home_hw,course2_away_hw,winner_player_id,is_draw)
    values(v_public_result.id,v_fixture.id,v_fixture.season_id,v_fixture.pyp_roster_version_id,v_fixture.division_number,v_fixture.game_number,v_fixture.pyp_home_player_id,v_fixture.pyp_away_player_id,
      v_fixture.pyp_home_player_screen_name,v_fixture.pyp_away_player_screen_name,btrim(p_course1_name),p_course1_home_hw,p_course1_away_hw,btrim(p_course2_name),p_course2_home_hw,p_course2_away_hw,v_winner,v_draw)
...
revoke all on function public.save_pyp_result(uuid,text,integer,integer,text,integer,integer) from public,anon,authenticated;
grant execute on function public.save_pyp_result(uuid,text,integer,integer,text,integer,integer) to authenticated;
