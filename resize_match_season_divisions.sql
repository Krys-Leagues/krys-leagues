create or replace function public.resize_match_season_divisions(
  p_season_id uuid,
...
begin
  ...
  if not found then
    if exists (
      ...
      raise exception 'A locked historical Match roster cannot be resized'
        using errcode = '42501';
    end if;

    raise exception 'No current draft or approved Match roster was found for this season';
  end if;

  perform 1
  from public.seasons as season
  where season.id = p_season_id
    and lower(btrim(season.league_type)) = 'match'
  for share;

  if not found then
    raise exception 'The requested season was not found or is not Match';
  end if;
  ...
  if v_previous_division_count is null or v_previous_division_count < 1 then
    raise exception 'The current Match roster has an invalid division count';
  end if;
  ...
  if (
    ...
    raise exception 'The current Match roster does not contain exactly four slots per division';
  end if;

  if exists (
    select 1
    from pg_catalog.generate_series(1, v_previous_division_count) as expected(division_number)
    left join public.match_division_roster_slots as slot
    ...
    raise exception 'The current Match roster does not contain exactly slots 1 through 4 for every division';
  end if;
  ...
  if v_roster.status = 'approved' then
    ...
    if not found then
      raise exception 'Approved Match roster has no schedule workflow state';
    end if;
  end if;
  ...
  if p_new_division_count > v_previous_division_count then
    ...
    update public.match_roster_versions as roster
    ...
    if exists (
      ...
      raise exception 'Cannot remove Match D% because it still contains roster players',
        (
    ...
    if exists (
      ...
      raise exception 'Cannot remove trailing Match divisions because completed results exist';
    end if;
    ...
    if exists (
      ...
      raise exception 'Cannot remove trailing Match divisions because result records exist';
    end if;
    ...
    delete from public.match_division_course_overrides as course_override
    where course_override.season_id = p_season_id
      and course_override.division_number > p_new_division_count;

    delete from public.match_division_roster_slots as slot
    where slot.roster_version_id = v_roster.id
      and slot.division_number > p_new_division_count;
...
revoke all on function public.resize_match_season_divisions(uuid, integer) from public;
revoke all on function public.resize_match_season_divisions(uuid, integer) from anon;
revoke all on function public.resize_match_season_divisions(uuid, integer) from authenticated;
grant execute on function public.resize_match_season_divisions(uuid, integer) to authenticated;
