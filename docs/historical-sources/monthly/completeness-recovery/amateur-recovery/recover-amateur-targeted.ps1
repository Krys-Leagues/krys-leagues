...
$root = Join-Path (Get-Location) 'docs\historical-sources\monthly\completeness-recovery\amateur-recovery'
$rawDir = Join-Path $root 'raw'
$utf8 = New-Object Text.UTF8Encoding($false)
New-Item -ItemType Directory -Force -Path $root, $rawDir | Out-Null

$divisions = @(
  [pscustomobject]@{ Label = 'Amateur 1'; Value = 26 },
  [pscustomobject]@{ Label = 'Amateur 2'; Value = 27 },
  [pscustomobject]@{ Label = 'Amateur 3'; Value = 28 }
)
$periodIds = @{}
...
$targetPeriodIds = @(54,55,56,57,58,81,101,121,141,161,181,201,221,241,261,281,301)
$targetCells = @('54+27','55+26','55+27','55+28','56+27','57+27','58+27','54+28','56+28','57+28','58+28','81+26','81+27','81+28','101+27','121+27','121+28','141+26','141+27','161+26','161+27','161+28','181+26','181+27','181+28','201+26','201+27','221+26','221+27','241+27','261+26','261+27','281+26','301+26','301+27')
...
function Navigate-Raw([string] $Url) { Send-NoWait 'Page.navigate' @{ url = $Url }; Start-Sleep -Milliseconds 3500 }
...
try {
  ...
  foreach ($p in $periods | Where-Object { $_.status -eq 'COMPLETED' -and $targetPeriodIds -contains $_.periodId } | Sort-Object periodId -Descending) {
    foreach ($d in $divisions) {
      $key = "$($p.periodId)+$($d.Value)"
      if ($done.ContainsKey($key)) { continue }; if (-not ($targetCells -contains $key)) { continue }
