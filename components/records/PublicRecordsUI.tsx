...
export function PublicRecordsHero({ title, description, eyebrow = "Krys Leagues · All-Time Records", subheading }: { title: string; description: string; eyebrow?: string; subheading?: string }) { return <header className={styles.hero}><Image className={styles.logo} src="/league-media/BIG LOGO TRANSPARENT.png" alt="Krys Leagues crest" width={72} height={72} priority/><div><p className={styles.eyebrow}>{eyebrow}</p><h1 className={styles.title}>{title}</h1>{subheading && <p className={styles.heroSubheading}>{subheading}</p>}<p className={styles.subtitle}>{description}</p></div></header> }
