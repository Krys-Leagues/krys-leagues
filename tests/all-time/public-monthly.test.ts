...
test("public Monthly results are browseable by year, month, and division", () => {
  assert.match(page, /Monthly Results/)
  assert.match(page, /label="Year"/)
  assert.match(page, /label="Month"/)
  assert.match(page, /label="Division"/)
  assert.match(page, /Overall standings \/ summary/)
  assert.match(page, /Course results/)
...
test("Player Profile loads Monthly history through the public canonical-player endpoint", () => {
  ...
  assert.match(profile, /Course-level Monthly scores/)
