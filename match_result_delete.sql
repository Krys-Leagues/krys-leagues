...
begin
  ...
  if v_user_id is null then
    raise exception
      'Authentication is required to delete a Match result'
      using errcode = '42501';
  end if;
  ...
  if exists (
    ...
    raise exception
      'Results cannot be deleted from a finalized Match season'
      using errcode = '42501';
  end if;
  ...
  if not found then
    raise exception
      'Match results can be deleted only while the current schedule is generated and reviewed';
  end if;
  ...
  perform 1
  from public.rebuild_match_standings(
    v_schedule.season_id,
    v_schedule.division_number
  );
...
revoke all on function public.delete_match_result(uuid)
from public;
...
grant execute on function public.delete_match_result(uuid)
to authenticated;
