CREATE OR REPLACE FUNCTION public.create_match_season_with_roster(p_season_number integer, p_division_count integer, p_start_date date, p_due_date date, p_end_date date, p_game1_course text, p_game2_course text, p_game3_course text)
...
begin
  ...
  if exists (
    ...
    where existing_season.league_type = 'match'
      and existing_season.season_number = p_season_number
  ) then
    raise exception 'Match Season % already exists', p_season_number
      using errcode = '23505';
  end if;
  ...
  insert into public.match_roster_versions (
    season_id,
    division_count,
    status,
...
revoke all on function public.create_match_season_with_roster(integer, integer, date, date, date, text, text, text) from public;
revoke all on function public.create_match_season_with_roster(integer, integer, date, date, date, text, text, text) from anon;
revoke all on function public.create_match_season_with_roster(integer, integer, date, date, date, text, text, text) from authenticated;
grant execute on function public.create_match_season_with_roster(integer, integer, date, date, date, text, text, text) to authenticated;
