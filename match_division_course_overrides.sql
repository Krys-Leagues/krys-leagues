CREATE OR REPLACE FUNCTION public.set_match_division_course_overrides(p_season_id uuid, p_division_number integer, p_game1_course text, p_game2_course text, p_game3_course text)
...
begin
  ...
  if not found then
    if exists (
      ...
      raise exception
        'Course overrides for a locked historical Match season cannot be changed'
        using errcode = '42501';
    end if;

    raise exception
      'No editable draft or approved Match roster was found for this season';
  end if;
  ...
  if v_season.league_type <> 'match' then
  ...
  from public.match_division_course_overrides as course_override
  ...
  if v_game1_override is null
    ...
    insert into public.match_division_course_overrides (
      season_id,
      division_number,
...
revoke all on function public.set_match_division_course_overrides(uuid, integer, text, text, text) from public;
revoke all on function public.set_match_division_course_overrides(uuid, integer, text, text, text) from anon;
revoke all on function public.set_match_division_course_overrides(uuid, integer, text, text, text) from authenticated;
grant execute on function public.set_match_division_course_overrides(uuid, integer, text, text, text) to authenticated;
