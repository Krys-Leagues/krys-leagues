create or replace function public.update_match_season_details(
  p_season_id uuid,
  p_start_date date,
  p_end_date date,
  p_game1_course text,
...
declare
  ...
  v_game1_course text := nullif(btrim(p_game1_course), '');
  v_game2_course text := nullif(btrim(p_game2_course), '');
  v_game3_course text := nullif(btrim(p_game3_course), '');
...
begin
  ...
  if v_game1_course is null or v_game2_course is null or v_game3_course is null then
    raise exception 'Game 1, Game 2, and Game 3 course names are required';
  end if;
  ...
  if not found then
    if exists (
      ...
      raise exception 'A locked historical Match season cannot be changed'
        using errcode = '42501';
    end if;

    raise exception 'No editable draft or approved Match roster was found for this season';
  end if;
  ...
  v_details_changed :=
    ...
    or v_season.game1_course is distinct from v_game1_course
    or v_season.game2_course is distinct from v_game2_course
    or v_season.game3_course is distinct from v_game3_course;
  ...
  if v_details_changed then
    ...
    set start_date = p_start_date,
        due_date = p_end_date,
        end_date = p_end_date,
        game1_course = v_game1_course,
        game2_course = v_game2_course,
        game3_course = v_game3_course
    where season.id = p_season_id
...
revoke all on function public.update_match_season_details(uuid, date, date, text, text, text) from public;
revoke all on function public.update_match_season_details(uuid, date, date, text, text, text) from anon;
revoke all on function public.update_match_season_details(uuid, date, date, text, text, text) from authenticated;
grant execute on function public.update_match_season_details(uuid, date, date, text, text, text) to authenticated;
