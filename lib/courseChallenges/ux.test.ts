...
test("full book conceals unearned reward artwork and preserves Ace Track wording", () => {
  ...
  assert.match(book, /Available from Level 1/)
  assert.match(book, /ACE TRACK/)
...
test("Tourist Trap book uses full-page scenery and starts with Rules / Help", () => {
  ...
  assert.doesNotMatch(book, /courseHero|Levels 1–5 are visible together/)
...
test("level rail is interactive single-view navigation with locked states", () => {
  ...
  assert.match(book, /aceStages\.length > 0/)
  assert.match(book, /bookCover/)
  assert.match(book, /bookPhase/)
...
test("welcome guide explains the purpose, progression, hints, and profile rewards", () => {
  ...
  assert.match(guide, /Ace Track/)
  assert.match(guide, /Levels 1–5/)
  ...
  assert.doesNotMatch(guide, /Becomes available after Level 3/)
  assert.doesNotMatch(guide, /Earned when you complete Level 5/)
})

test("the course book visibly animates its cover open before revealing the pages", () => {
  const book = read("components/course-challenges/CourseChallengeBook.tsx")
  const styles = read("components/course-challenges/course-challenges.module.css")
  assert.match(book, /data-book-phase=\{bookPhase\}/)
  assert.match(book, /onAnimationEnd=\{\(\) => setBookPhase\("open"\)\}/)
  assert.match(styles, /bookCoverOpen/)
  assert.match(styles, /rotateY\(-102deg\)/)
  assert.match(styles, /bookPagesReveal/)
  assert.match(styles, /bookSpread::after/)
  assert.match(styles, /\.bookSpread::after \{ display: none;/)
  assert.match(styles, /@container \(min-width: 1040px\)/)
  assert.match(styles, /left: calc\(\(100% - 18px\) \/ 3 \+ 9px\)/)
  assert.doesNotMatch(book, /currentLevel === 3/)
...
test("admin Course Challenge review queue exposes pending count and review controls", () => {
  const admin = read("components/admin/course-challenges/CourseChallengeReviewQueue.tsx")
  const adminPage = read("app/admin/course-challenges/page.tsx")
  const api = read("app/api/admin/course-challenges/route.ts")
  const dashboard = read("app/admin/page.tsx")
  assert.match(adminPage, /Course Challenge Review Queue/)
  assert.match(adminPage, /Admin Dashboard/)
  assert.match(admin, /Pending reviews/)
  assert.match(admin, /APPROVE/)
  assert.match(admin, /REJECT/)
  assert.match(admin, /PAST CARDS/)
  assert.match(admin, /Open original private proof/)
  assert.match(admin, /Private scorecard evidence/)
  assert.match(admin, /P\{submission\.pars\?\.\[index\]/)
  assert.match(admin, /Review reason:/)
  assert.match(api, /requireCourseChallengeAdmin/)
  assert.match(api, /params\.get\("summary"\)/)
  assert.match(api, /pendingCount/)
  assert.match(api, /status.*pending.*needs_review/)
  assert.match(dashboard, /COURSE CHALLENGE SUBMISSIONS/)
  assert.match(dashboard, /api\/admin\/course-challenges\?summary=1/)
})
