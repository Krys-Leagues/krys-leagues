...
export function normalizeCanonicalCourseDisplayName(displayName: string) {
  return displayName.replaceAll("Widowâ€™s Walkabout", "Widow’s Walkabout")
}

export function formatCanonicalCourse<T extends CanonicalCourseOption>(course: T) {
  const displayName = normalizeCanonicalCourseDisplayName(course.display_name).trim()
