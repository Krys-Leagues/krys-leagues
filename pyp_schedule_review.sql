create or replace function public.review_pyp_schedule(
  p_season_id uuid
)
returns table(
  season_id uuid,
...
begin
  ...
  if v_user_id is null then
    raise exception
      'Authentication is required to review a PYP schedule'
      using errcode = '42501';
  end if;
  ...
  if not found then
    if exists (
      ...
      raise exception
        'A locked historical PYP roster cannot have its schedule reviewed'
        using errcode = '42501';
    end if;

    raise exception
      'Exactly one approved PYP roster is required before schedule review';
  end if;
  ...
  if v_generated_revision <> v_change_revision then
    raise exception
      'The PYP schedule is stale and must be generated before review';
  end if;

  if v_generated_revision <= 0 then
    raise exception
      'The PYP schedule has not been generated and cannot be reviewed';
  end if;

  -- Lock and count only managed PYP fixtures. A count of zero is valid.

  perform 1
  from public.schedule as managed_fixture
  where lower(btrim(managed_fixture.league_type)) = 'pyp'
    and managed_fixture.season_id = p_season_id
    and managed_fixture.pyp_roster_version_id = v_roster.id
...
revoke all
  on function public.review_pyp_schedule(uuid)
  from public;
