...
begin
  ...
  for v_row in select value from jsonb_array_elements(p_rows) loop
    ...
    if exists(select 1 from public.historical_kwt_scorecards where source_fingerprint=v_row->>'rowKey' and historical_kwt_import_id<>v_import.id) then raise exception 'KWT source fingerprint % conflicts with an existing import',v_row->>'rowKey';end if;
