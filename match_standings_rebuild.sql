create or replace function public.rebuild_match_standings(
  p_season_id uuid,
  p_division_number integer
)
returns table(
  season_id uuid,
  match_roster_version_id uuid,
...
begin
  ...
  if v_user_id is null then
    raise exception
      'Authentication is required to rebuild Match standings'
      using errcode = '42501';
  end if;
  ...
  if not found then
    if exists (
      select 1
      from public.match_roster_versions as locked_roster
      where locked_roster.season_id = p_season_id
        and locked_roster.status = 'locked'
    ) then
      raise exception
        'A locked historical Match roster cannot have live standings rebuilt'
        using errcode = '42501';
    end if;

    raise exception 'Exactly one approved Match roster is required';
  end if;
  ...
  if v_approved_roster_count <> 1 then
    raise exception
      'Exactly one approved Match roster is required; found %',
  ...
  if exists (
    ...
    raise exception
      'A locked historical Match roster exists for this season; live standings rebuild is not allowed'
      using errcode = '42501';
  end if;
  ...
  if lower(btrim(v_season.league_type)) is distinct from 'match' then
    raise exception 'Season % is not a Match season', p_season_id;
  end if;
  ...
  if not found then
    raise exception
      'Match standings can be rebuilt only after the current schedule has been generated and reviewed';
  end if;
  ...
  if exists (
    ...
    raise exception
      'Every populated Match roster slot must have a screen-name snapshot';
  end if;
  ...
  if exists (
    ...
    where lower(btrim(result_row.league_type)) = 'match'
      and lower(btrim(scheduled_fixture.league_type)) = 'match'
      and scheduled_fixture.season_id = p_season_id
      and scheduled_fixture.match_roster_version_id = v_roster.id
      and scheduled_fixture.division_number = p_division_number
      and result_row.player1_hw is not null
      and result_row.player2_hw is not null
      and (
        ...
        or not exists (
          select 1
          from public.match_division_roster_slots as player1_slot
        ...
        or not exists (
          select 1
          from public.match_division_roster_slots as player2_slot
    ...
    raise exception
      'A managed completed Match result does not match the approved roster or its schedule fixture';
  end if;

  v_division := 'Match D' || p_division_number::text;
  ...
  where standing.league_type = 'match'
  ...
  for v_standing in
    ...
    contributions as (
      select
        ...
        case when completed_result.player1_hw > completed_result.player2_hw then 1 else 0 end as wins,
        case when completed_result.player1_hw < completed_result.player2_hw then 1 else 0 end as losses,
        case when completed_result.player1_hw = completed_result.player2_hw then 1 else 0 end as ties,
        completed_result.player1_hw as holes_won
      ...
      select
        ...
        case when completed_result.player2_hw > completed_result.player1_hw then 1 else 0 end as wins,
        case when completed_result.player2_hw < completed_result.player1_hw then 1 else 0 end as losses,
        case when completed_result.player2_hw = completed_result.player1_hw then 1 else 0 end as ties,
        completed_result.player2_hw as holes_won
      from completed_results as completed_result
    ),
    totals as (
      select
        ...
        coalesce(sum(contribution.holes_won), 0)::integer as holes_won
    ...
    head_to_head as (
      select
        ...
        coalesce(sum(
          case
            when completed_result.player1_id = total.player_id then
              case when completed_result.player1_hw > completed_result.player2_hw then 3
                   when completed_result.player1_hw = completed_result.player2_hw then 1 else 0 end
            else
              case when completed_result.player2_hw > completed_result.player1_hw then 3
                   when completed_result.player2_hw = completed_result.player1_hw then 1 else 0 end
          end
        ) filter (where opponent.wins * 3 + opponent.ties = total.wins * 3 + total.ties), 0)::integer as head_to_head_points
      from totals as total
      left join completed_results as completed_result
        on total.player_id in (completed_result.player1_id, completed_result.player2_id)
      left join totals as opponent
        on opponent.player_id = case
          when completed_result.player1_id = total.player_id then completed_result.player2_id
          else completed_result.player1_id
        end
      group by total.player_id
    )
    select
      total.player_id,
      total.wins,
      ...
      row_number() over (
        order by
          ...
          head_to_head.head_to_head_points desc,
          (total.completed_game_count > 0) desc,
          total.holes_won desc,
          total.player_screen_name asc
      )::integer as rank
    from totals as total
    join head_to_head on head_to_head.player_id = total.player_id
  loop
...
revoke all on function public.rebuild_match_standings(uuid, integer)
from public;
...
grant execute on function public.rebuild_match_standings(uuid, integer)
to authenticated;
