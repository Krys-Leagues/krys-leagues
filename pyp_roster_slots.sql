create or replace function public.set_pyp_division_roster_slots(
...
begin
  ...
  if auth.uid() is null then
    raise exception
      'Authentication is required to edit a PYP roster'
      using errcode = '42501';
  end if;
  ...
  if not found then
    raise exception 'PYP roster version was not found';
  end if;

  if v_roster.status = 'locked' then
    raise exception
      'A locked historical PYP roster cannot be changed'
      using errcode = '42501';
  end if;

  if v_roster.status = 'cancelled' then
    raise exception
      'A cancelled PYP roster cannot be changed'
      using errcode = '42501';
  end if;

  if v_roster.status not in ('draft', 'approved') then
    raise exception 'This PYP roster is not editable';
  end if;
  ...
  where season.id = v_roster.season_id
    and season.league_type = 'pyp';

  if not found then
    raise exception
      'The roster does not reference a valid PYP season';
  end if;
  ...
  if v_roster.status = 'approved' then
    ...
    if not found then
      raise exception
        'Approved PYP roster has no schedule workflow state';
    end if;
  end if;
  ...
  -- at least one persisted completed real PYP game.
  --
  ...
  from public.pyp_division_roster_slots as existing_slot
  where existing_slot.roster_version_id = v_roster.id
    ...
    and exists (
      ...
      where completed_result.league_type = 'pyp'
  ...
  if found then
    raise exception
      'Player % has completed a PYP game and cannot be removed, replaced, or moved',
  ...
  update public.pyp_division_roster_slots as source_slot
  set
    player_id = null,
  ...
  update public.pyp_division_roster_slots as target_slot
  set
    player_id = null,
    player_screen_name = null,
    slot_status = 'empty'
  ...
  if not found then
    raise exception
      'The target PYP roster slots could not be updated';
  end if;
  ...
  from public.pyp_division_roster_slots as saved_slot
...
revoke all
  on function public.set_pyp_division_roster_slots(
    uuid,
    integer,
    uuid,
    uuid,
