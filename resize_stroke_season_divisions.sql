create or replace function public.resize_stroke_season_divisions(
  p_season_id uuid,
...
begin
  ...
  if not found then
    if exists (
      ...
      raise exception 'A locked historical Stroke roster cannot be resized'
        using errcode = '42501';
    end if;

    raise exception 'No current draft or approved Stroke roster was found for this season';
  end if;
  ...
  if not found then
    raise exception 'The requested season was not found or is not Stroke';
  end if;
  ...
  if v_previous_division_count is null or v_previous_division_count < 1 then
    raise exception 'The current Stroke roster has an invalid division count';
  end if;
  ...
  if (
    ...
    raise exception 'The current Stroke roster does not contain exactly four slots per division';
  end if;

  if exists (
    select 1
    from pg_catalog.generate_series(1, v_previous_division_count) as expected(division_number)
    left join public.stroke_division_roster_slots as slot
    ...
    raise exception 'The current Stroke roster does not contain exactly slots 1 through 4 for every division';
  end if;
  ...
  if v_roster.status = 'approved' then
    ...
    if not found then
      raise exception 'Approved Stroke roster has no schedule workflow state';
    end if;
  end if;
  ...
  if p_new_division_count > v_previous_division_count then
    ...
    update public.stroke_roster_versions as roster
    ...
    if exists (
      ...
      raise exception 'Cannot remove Stroke D% because it still contains roster players',
        (
    ...
    where lower(btrim(fixture.league_type)) = 'stroke'
      and fixture.season_id = p_season_id
      and fixture.roster_version_id = v_roster.id
    ...
    if exists (
      ...
      where lower(btrim(fixture.league_type)) = 'stroke'
        ...
        and result.player1_score is not null
        and result.player2_score is not null
    ) then
      raise exception 'Cannot remove trailing Stroke divisions because completed results exist';
    end if;
    ...
    if exists (
      ...
      raise exception 'Cannot remove trailing Stroke divisions because result records exist';
    end if;
    ...
    delete from public.stroke_division_roster_slots as slot
    where slot.roster_version_id = v_roster.id
      and slot.division_number > p_new_division_count;
...
revoke all on function public.resize_stroke_season_divisions(uuid, integer) from public;
revoke all on function public.resize_stroke_season_divisions(uuid, integer) from anon;
revoke all on function public.resize_stroke_season_divisions(uuid, integer) from authenticated;
grant execute on function public.resize_stroke_season_divisions(uuid, integer) to authenticated;
