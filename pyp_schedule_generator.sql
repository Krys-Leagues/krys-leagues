create or replace function public.generate_pyp_schedule(
  p_season_id uuid
)
returns table(
  season_id uuid,
...
begin
  ...
  if v_user_id is null then
    raise exception
      'Authentication is required to generate a PYP schedule'
      using errcode = '42501';
  end if;
  ...
  -- Lock the approved roster first. This follows the shared PYP lock
  ...
  if not found then
    if exists (
      ...
      raise exception
        'A locked historical PYP roster cannot have its schedule regenerated'
        using errcode = '42501';
    end if;

    raise exception
      'Exactly one approved PYP roster is required before schedule generation';
  end if;
  ...
  if v_approved_roster_count <> 1 then
    raise exception
      'Exactly one approved PYP roster is required; found %',
  ...
  if v_roster.division_count is null or v_roster.division_count <= 0 then
    raise exception
      'The approved PYP roster must have a positive division count';
  end if;
  ...
  if not found then
    raise exception
      'The approved PYP roster has no schedule workflow state';
  end if;
  ...
  perform 1
  from public.pyp_division_roster_slots as roster_slot
  ...
  if exists (
    ...
    left join public.pyp_division_roster_slots as roster_slot
  ...
  if v_generated_revision = v_change_revision then
    ...
    where lower(btrim(scheduled_fixture.league_type)) = 'pyp'
      and scheduled_fixture.season_id = p_season_id
    and scheduled_fixture.pyp_roster_version_id = v_roster.id;
    ...
    and scheduled_fixture.pyp_roster_version_id = v_roster.id
      and exists (
        ...
        where completed_result.schedule_id = scheduled_fixture.id
          and lower(btrim(completed_result.league_type)) = 'pyp'
  ...
  -- Lock only managed PYP fixtures for this season. Legacy/test rows
    ...
    and scheduled_fixture.pyp_roster_version_id = v_roster.id
      ...
      and not exists (
        ...
        where desired_fixture.division_number =
          ...
          and desired_fixture.player1_id = scheduled_fixture.pyp_home_player_id
          and desired_fixture.player2_id = scheduled_fixture.pyp_away_player_id
      )
  ) then
    raise exception
      'A completed PYP fixture conflicts with the current approved roster; completed history was not changed';
  end if;
  ...
  if exists (
    ...
    raise exception
      'An obsolete PYP fixture has a linked result and cannot be removed safely';
  end if;
  ...
  set
    ...
    division = 'PYP D' || desired_row.division_number::text,
    ...
    player1_id = desired_row.player1_id,
    player2_id = desired_row.player2_id,
    pyp_home_player_id = desired_row.player1_id,
    pyp_away_player_id = desired_row.player2_id,
    pyp_home_player_screen_name = desired_row.player1_screen_name,
    pyp_away_player_screen_name = desired_row.player2_screen_name,
    due_date = v_season.due_date,
    roster_version_id = null,
    pyp_roster_version_id = v_roster.id,
    division_number = desired_row.division_number,
    game_number = desired_row.game_number
  from desired_rows as desired_row
  where lower(btrim(scheduled_fixture.league_type)) = 'pyp'
    ...
    and row(
      ...
      scheduled_fixture.player1_id,
      scheduled_fixture.player2_id,
      scheduled_fixture.pyp_home_player_id,
      scheduled_fixture.pyp_away_player_id,
      scheduled_fixture.pyp_home_player_screen_name,
      scheduled_fixture.pyp_away_player_screen_name,
      scheduled_fixture.due_date,
      scheduled_fixture.pyp_roster_version_id,
      scheduled_fixture.division_number,
      scheduled_fixture.game_number
    ) is distinct from row(
      'pyp'::text,
      v_season.season_number,
      'PYP D' || desired_row.division_number::text,
  ...
  insert into public.schedule (
    ...
    pyp_home_player_id,
    pyp_away_player_id,
    pyp_home_player_screen_name,
    pyp_away_player_screen_name,
    status,
    due_date,
    season_id,
    pyp_roster_version_id,
    division_number,
    game_number
  )
  select
  ...
  where not exists (
    ...
    where lower(btrim(existing_fixture.league_type)) = 'pyp'
      and existing_fixture.season_id = p_season_id
      and existing_fixture.pyp_roster_version_id = v_roster.id
...
revoke all
  on function public.generate_pyp_schedule(uuid)
  from public;
