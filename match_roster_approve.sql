CREATE OR REPLACE FUNCTION public.approve_match_roster_version(p_roster_version_id uuid, p_approval_note text DEFAULT NULL::text)
...
begin
  ...
  into
    ...
    v_locked_at,
    v_locked_by
  from public.match_roster_versions rv
  where rv.id = p_roster_version_id
  for update;

  if not found then
    raise exception
      'Match roster version % was not found.',
      p_roster_version_id;
  end if;
  ...
  where s.id = v_season_id
    and lower(btrim(s.league_type)) = 'match'
  for update;

  if not found then
    raise exception
      'Roster version % does not belong to a valid Match season.',
      p_roster_version_id;
  end if;
  ...
  if v_roster_status <> 'draft' then
    raise exception
      'Only a draft Match roster can be approved. Current status: %.',
      v_roster_status;
  end if;

  if v_locked_at is not null or v_locked_by is not null then
    raise exception
      'This Match roster is historically locked and cannot be approved.';
  end if;

  if v_division_count is null or v_division_count <= 0 then
    raise exception
      'The Match roster must have a positive division count.';
  end if;
  ...
  perform 1
  from public.match_division_roster_slots rs
  ...
  if exists (
    ...
    left join public.match_division_roster_slots rs
  ...
  update public.match_roster_versions rv
  set
    status = 'approved',
    approved_at = now(),
  ...
  insert into public.match_schedule_state (
    season_id,
    change_revision,
    generated_revision,
    reviewed_revision,
  ...
  on conflict on constraint match_schedule_state_pkey do nothing;
  ...
  into
    ...
    v_posted_at,
    v_posted_by
  from public.match_schedule_state ss
  where ss.season_id = v_season_id
  for update;

  if not found then
    raise exception
      'Unable to initialize schedule workflow state for Match season %.',
      v_season_id;
  end if;
...
revoke all on function public.approve_match_roster_version(uuid, text) from public;
revoke all on function public.approve_match_roster_version(uuid, text) from anon;
revoke all on function public.approve_match_roster_version(uuid, text) from authenticated;
grant execute on function public.approve_match_roster_version(uuid, text) to authenticated;
