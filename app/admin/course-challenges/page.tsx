  ...
  return <main style={{ minHeight: "100vh", padding: 28, background: "#020617", color: "#f8fafc" }}>
    <div style={{ maxWidth: 1280, margin: "0 auto" }}>
      ...
      {loading ? <p>Loading reviews…</p> : submissions.length === 0 ? <p>No pending Course Challenge submissions.</p> : <div style={{ display: "grid", gap: 18 }}>{submissions.map((submission) => <article key={submission.id} style={{ border: "1px solid #334155", borderRadius: 16, padding: 18, background: "#0f172a" }}>
        <header style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", gap: 10 }}><div><h2 style={{ margin: 0 }}>{submission.playerName} · {submission.courseName}</h2><p style={{ color: "#cbd5e1" }}>{submission.challengeKey === "ace" ? "Ace Challenge" : "Level " + submission.level} · {submission.difficulty} · {submission.roundDate} {submission.roundTime}</p></div><strong>{submission.status}</strong></header>
        ...
        <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{submission.holeScores.map((_, index) => <th key={index} style={{ padding: 6, color: "#94a3b8" }}>H{index + 1}</th>)}<th>Total</th><th>To par</th></tr></thead><tbody><tr>{submission.holeScores.map((score, index) => <td key={index} style={{ padding: 6, textAlign: "center" }}>{score}<small style={{ display: "block", color: "#a5b4fc" }}>P{submission.pars?.[index] ?? "—"}</small></td>)}<td style={{ textAlign: "center" }}>{submission.calculatedTotal}</td><td style={{ textAlign: "center" }}>{submission.relativeToPar > 0 ? `+${submission.relativeToPar}` : submission.relativeToPar}</td></tr></tbody></table></div>
        <ul>{submission.requirements.map((item, index) => <li key={index}>{item.requirement?.label || "Requirement"}: {item.status}{item.reason ? ` — ${item.reason}` : ""}</li>)}</ul>
