...
{season?<><Link href={`/admin/pyp/setup?seasonId=${season.id}&division=1`} style={card}><b>Setup / Rosters</b><span>Edit Season {season.season_number} roster divisions.</span></Link><Link href={`/admin/pyp/schedule?seasonId=${encodeURIComponent(season.id)}`} style={card}><b>Schedule &amp; Images</b><span>View the current schedule, review changes, and download division schedule images.</span></Link></>:<><div style={{...card,opacity:.6}}><b>Setup / Rosters</b><span>No current managed PYP season is available yet.</span></div><div style={{...card,opacity:.6}}><b>Schedule &amp; Images</b><span>No current managed PYP season is available yet.</span></div></>}
