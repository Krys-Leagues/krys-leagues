...
type Season = { id: string; season_number: number; start_date: string | null; end_date: string | null }
...
type TransitionDraft = { divisionCount: string; startDate: string; endDate: string }

const transitionDraftKey = (sourceSeasonId: string) => `pyp-transition-season-draft:${sourceSeasonId}`
const transitionPlayersKey = (sourceSeasonId: string) => `pyp-transition-new-players:${sourceSeasonId}`
...
export default function PYPTransitionPage() {
  ...
  useEffect(() => {
    ...
    const draft: TransitionDraft = { divisionCount, startDate, endDate }
    window.sessionStorage.setItem(transitionDraftKey(sourceSeasonId), JSON.stringify(draft))
  }, [divisionCount, endDate, sourceSeasonId, startDate, targetSeasonId])
  ...
  async function createNextSeason() {
    ...
    if (!Number.isInteger(seasonNumber) || seasonNumber <= 0) { setError("The next PYP season number could not be determined."); return }
    ...
    if (createError || !data) {
      ...
      await logActivity({ userType: "admin", action: "Create PYP Season From Transition Failed", status: "error", leagueType: "pyp", page: "/admin/pyp/transition", details: { sourceSeasonId, seasonNumber, divisionCount: parsedCount, error: errorMessage } })
      setError(`PYP season was not created: ${errorMessage}`); setCreatingSeason(false); return
    }
    ...
    await logActivity({ userType: "admin", action: "Created PYP Season From Transition", status: "success", leagueType: "pyp", page: "/admin/pyp/transition", details: { sourceSeasonId, targetSeasonId: created.season_id, seasonNumber: created.season_number, divisionCount: created.division_count } })
    setMessage(`PYP Season ${created.season_number} created and selected as the target season. Return decisions and new-player order were preserved.`)
  ...
  if (loading) return <main style={page}><p>Loading PYP transition...</p></main>
  return <main style={page}><div style={container}>
    <div style={actions}>
      <button onClick={() => router.push("/admin/pyp")} style={secondaryButton}>← PYP Hub</button>
      <button onClick={() => router.push(sourceSeasonId ? `/admin/pyp/standings?seasonId=${encodeURIComponent(sourceSeasonId)}` : "/admin/pyp/standings")} style={secondaryButton}>Scorecard / Standings</button>
      <button onClick={() => router.push(sourceSeasonId ? `/admin/pyp/schedule?seasonId=${encodeURIComponent(sourceSeasonId)}` : "/admin/pyp/schedule")} style={secondaryButton}>Schedule &amp; Images</button>
    </div>
    <h1>Next-Season PYP Transition</h1>
    ...
    <section style={panel}><h2>Returning Decisions</h2>
      {sourceDivisions.map((division) => {
        ...
        return <div key={division} style={{ ...divisionCard, background: theme?.background || "#0b0b0b", borderColor: theme?.border || "#444" }}>
          <h3 style={{ color: theme?.accent || "#ddd" }}>PYP D{division}</h3>
    ...
    <section style={panel}><h2>Create Next PYP Season</h2>
      <p>The normal path is to create and select the next managed PYP season here.</p>
      {targetSeasonId && <p style={successText}>Managed PYP Season {targetSeasons.find((season) => season.id === targetSeasonId)?.season_number} is selected automatically.</p>}
      <div style={formRow}>
      ...
      <button onClick={() => void createNextSeason()} disabled={creatingSeason || Boolean(targetSeasonId)} style={primaryButton}>{creatingSeason ? "Creating Next PYP Season..." : targetSeasonId ? "Next PYP Season Ready" : "Create Next PYP Season"}</button>
    </section>
    ...
    {proposal.length > 0 && <section style={panel}><h2>Proposed Roster — DRAFT / NOT OFFICIAL</h2>
      {divisions.map((division) => { const theme = divisionThemes[division]; return <div key={division} style={{ ...divisionCard, background: theme?.background || "#0b0b0b", borderColor: theme?.border || "#444" }}><h3 style={{ color: theme?.accent || "#ddd" }}>PYP D{division}</h3>
      ...
      <button onClick={() => router.push(`/admin/pyp/setup?seasonId=${targetSeasonId}&division=1`)} style={primaryButton}>Continue to Draft Roster Setup</button>
    </section>}
