  ...
  useEffect(() => {
    async function loadStrokeSeasons() {
      ...
      if (response.error || !response.data) {
        setErrorMessage(`Could not load Stroke seasons: ${response.error?.message || "No data returned."}`)
        setLoading(false)
        return
      }
  ...
  async function saveSeasonDetails() {
    ...
    try {
      if (requestedDivisionCount !== selectedRoster.division_count) {
        const { data: resizeData, error: resizeError } = await adminManagedLeagueRequest<ResizedRoster>("rpc", { name: "resize_stroke_season_divisions", args: {
      ...
      const { data, error } = await adminManagedLeagueRequest<SavedSeasonDetails>("rpc", { name: "update_stroke_season_details", args: {
          p_season_id: selectedSeason.id,
