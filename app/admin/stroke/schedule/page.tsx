...
function renderScheduleImage(data: ScheduleImageData) {
  ...
  context.fillText("KRYS LEAGUES  /  STROKE", 68, 78)
...
export default function StrokeScheduleReviewPage() {
  ...
  async function loadSchedule() {
    ...
    const [stateResponse, fixtureResponse, slotResponse, overrideResponse] = await Promise.all([
      ...
      fetchAdminDivisionCourseOverrides("stroke", requestedSeasonId),
    ])
    ...
    if (fixtureIds.length > 0) {
      const resultResponse = await fetch(`/api/admin/players?view=results&league_type=stroke&schedule_ids=${encodeURIComponent(fixtureIds.join(","))}`, { credentials: "same-origin", cache: "no-store" })
  ...
  async function downloadScheduleImage(divisionNumber: number) {
    ...
    try {
      ...
      anchor.href = url
      anchor.download = `stroke-s${season.season_number}-d${divisionNumber}-schedule.png`
  ...
  return (
    <main style={page}>
                      ...
                      >
                        ...
                        <div style={imageActions}>
                          ...
                          >
                            {isRendering
                              ? "Creating PNG..."
                              : `Download Stroke D${divisionNumber} Image`}
                          </button>
                        </div>
                      </div>
                    )
      ...
      {previewDivision !== null && previewImageUrl && (
        <div style={previewOverlay} role="dialog" aria-modal="true" aria-label={`Stroke D${previewDivision} schedule image preview`}>
          <div style={previewPanel}>
            <div style={previewHeader}>
              <div>
                <strong>Stroke D{previewDivision} Schedule Image</strong>
