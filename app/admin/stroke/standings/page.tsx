...
export default function StrokeStandingsPage() {
  ...
  async function loadFinalScorecard(activeSeason: SeasonRow, requestId: number) {
    ...
    setScorecardError("")
    const response = await fetch(`/api/admin/stroke/final-scorecard?seasonId=${encodeURIComponent(activeSeason.id)}`, { cache: "no-store" })
      ...
      <div style={container}>
        ...
        <div style={controls}>
          ...
          <select value={divisionNumber} onChange={(event) => setDivisionNumber(event.target.value)} style={input}>
            {divisions.map((item) => (
              <option key={item} value={item}>Stroke D{item}</option>
            ))}
          </select>
          <button
        ...
        <section style={finalSection}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={tableWrap}>
                      <table style={table}>
                        <thead><tr>
                          <th style={th}>Rank</th><th style={th}>Player</th><th style={th}>Points</th>
                          <th style={th}>Wins</th><th style={th}>Losses</th><th style={th}>Ties</th>
                          <th style={th}>Strokes</th><th style={th}>Completed Games</th>
                        </tr></thead>
                        <tbody>
          ...
          {scorecard?.status === "approved" && (
            <button onClick={() => router.push(`/admin/stroke/transition?scorecardId=${scorecard.id}`)} style={generateButton}>
