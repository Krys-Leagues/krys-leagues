...
export default function StrokeTransitionPage() {
  ...
  async function loadTransition() {
    ...
    const response = await adminManagedLeagueRequest<{
      ...
      players: Player[]
    }>("transition_load", { league: "stroke", scorecardId: requestedId })
    ...
    if (loadedSeasons.length > 1) { setError("Multiple eligible managed Stroke target seasons were found. Resolve the ambiguity before continuing."); setLoading(false); return }
  ...
  async function setDecision(playerId: string, decision: Decision["decision"]) {
    ...
    const { error: saveError } = await adminManagedLeagueRequest("rpc", { name: "set_stroke_return_decision", args: {
  ...
  async function createNextSeason() {
    ...
    const { data, error: createError } = await adminManagedLeagueRequest("rpc", { name: "create_stroke_season_with_roster", args: {
  ...
  async function generateProposal() {
    ...
    const { data, error: generateError } = await adminManagedLeagueRequest("rpc", { name: "generate_stroke_next_season_proposal", args: {
