...
type Course = { id: string; code: string; display_name: string; difficulty: "Easy" | "Hard"; hole_pars: number[] | null }
type Player = { id: string; screen_name: string }
type Observation = { id: string; course_id: string; player_id: string | null; historical_player_name: string; score: number; entry_type: string; hole_strokes: number[] | null; source_label: string | null; provenance_reference: string | null; notes: string | null; observed_at: string; updated_at: string; recorded_by: string | null; voided_at: string | null; voided_by: string | null; void_reason: string | null; corrected_at: string | null }
...
export default function RecordsHistoryPage() {
  ...
  const [selected, setSelected] = useState<Observation | null>(null), [mode, setMode] = useState<"edit" | "void" | null>(null), [newScore, setNewScore] = useState(""), [newHoles, setNewHoles] = useState<string[]>([]), [reason, setReason] = useState("")
  const [loading, setLoading] = useState(true), [busy, setBusy] = useState(false), [error, setError] = useState(""), [message, setMessage] = useState("")

  async function load() {
    setLoading(true); setError("")
    const [courseResult, playerResult, rowResult] = await Promise.all([
      supabase.from("all_time_courses").select("id,code,display_name,difficulty,hole_pars").eq("active", true).in("difficulty", ["Easy", "Hard"]).order("display_name"),
      supabase.from("players").select("id,screen_name").eq("active", true).order("screen_name"),
      supabase.from("all_time_record_observations").select("id,course_id,player_id,historical_player_name,score,entry_type,hole_strokes,source_label,provenance_reference,notes,observed_at,updated_at,recorded_by,voided_at,voided_by,void_reason,corrected_at").order("observed_at", { ascending: false }),
    ])
  ...
  function openEdit(row: Observation) { setSelected(row); setMode("edit"); setReason(""); setNewScore(String(row.score)); setNewHoles(Array.isArray(row.hole_strokes) ? row.hole_strokes.map(String) : []) }
  async function submitCorrection() {
    ...
    const result = await supabase.rpc("correct_all_time_record_entry", { p_observation_id: selected.id, p_expected_updated_at: selected.updated_at, p_new_score: correctedScore, p_new_hole_strokes: fullCard ? correctedHoles : null, p_reason: reason.trim() })
  ...
  async function submitVoid() {
    ...
    setBusy(true); setError("")
    const result = await supabase.rpc("void_all_time_record_entry", { p_observation_id: selected.id, p_expected_updated_at: selected.updated_at, p_reason: reason.trim() })
  ...
  return <AdminRecordsShell>
    <nav className={styles.nav}><a href="/admin/records" className={styles.button}>← Records hub</a><a href="/admin/records/entry" className={styles.button}>Add entry</a><a href="/admin/records/climbers" className={styles.button}>Climbers</a></nav>
