...
type VerifiedPeriodPreview = { confirmation_token?: string; all_time_classification?: RecordClassification; current_pb_score?: number | null; new_pb_score?: number | null; climbers_points?: number | null; climbers_status?: string; target_period_label?: string }
...
export default function NormalRecordsEntryPage() {
  ...
  const targetPeriod = period === "current" ? season ? `${new Date(season.starts_at).toLocaleDateString()}–${new Date(season.ends_at).toLocaleDateString()}` : "Current period · no active season (0 points)" : selectedVerifiedSeason ? `${new Date(selectedVerifiedSeason.starts_at).toLocaleDateString()}–${new Date(selectedVerifiedSeason.ends_at).toLocaleDateString()}` : period === "previous" ? "Previous period · unavailable" : "Two periods ago · unavailable"
  ...
  function validateEntry() {
    ...
    if (isVerifiedPeriod && !periodPreview?.confirmation_token) return "Preview the selected completed period before saving."
  ...
  function validateEntryForPreview() {
    ...
    if (isVerifiedPeriod && selectedVerifiedSeason && (verifiedDate < selectedVerifiedSeason.starts_at.slice(0, 10) || verifiedDate >= selectedVerifiedSeason.ends_at.slice(0, 10))) return "The source-post date must fall inside the selected Climbers period."
    return null
  }
  ...
  function resetEntry(action: FastEntryAction) {
    ...
    setSource(next.source); setReference(next.reference); setNotes(next.notes); setVerifiedDate(next.verifiedDate); setVerifiedOrder(next.verifiedOrder); setBest(null); setCourseBests([]); setConfirmed(false); setPeriodPreview(null); setPreviewFingerprint("")
  ...
  return <AdminRecordsShell>
    ...
    <AdminGlassCard>
      ...
      {isVerifiedPeriod && <div className="mt-5 rounded-xl border border-amber-300/30 bg-amber-950/20 p-4"><strong className="block text-sm text-amber-100">AUTHORITATIVE BACKLOG CHRONOLOGY</strong><p className="mt-1 text-xs text-slate-300">Use the date the result was posted/submitted and its source-backed order on that date. The admin entry time is never used for Climbers replay.</p><div className="mt-3 grid gap-4 sm:grid-cols-2"><label className={styles.field}>Source-post date<input className={styles.input} type="date" value={verifiedDate} onChange={(event) => { setVerifiedDate(event.target.value); invalidatePreview() }} /></label><label className={styles.field}>Posting/source order<input className={styles.input} inputMode="numeric" pattern="[0-9]*" value={verifiedOrder} onChange={(event) => { setVerifiedOrder(event.target.value.replace(/\D/g, "")); invalidatePreview() }} placeholder="1, 2, 3…" /></label></div><p className="mt-2 text-xs text-amber-100">ADD AGAIN advances the source order. ADD AGAIN SC keeps the same date/order because every player on that physical card is replayed from one shared PB snapshot.</p></div>}
