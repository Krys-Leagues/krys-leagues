...
export default function MonthlyWebsiteImporter() {
  ...
  const validRows = payload?.rows.filter(row => row.importable && row.score !== null && row.issues.length === 0 && effective(identityKey(row.historicalPlayerName))) ?? []
  ...
  return <main className="mx-auto max-w-7xl p-6 text-white">
    ...
    <section className="mt-6 rounded-xl border border-zinc-700 bg-zinc-900 p-5"><h2 className="text-xl font-bold">Commit reviewed Monthly observations</h2><p className="mt-2 text-zinc-300">Only completed-period scored rows with a canonical UUID are sent to the installed RPC. Missing-score rows and all current/incomplete period rows remain blocked and are never sent.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><Stat label="Resolved names" value={names.length - needsReview.length} /><Stat label="Ready observations" value={validRows.length} /><Stat label="Blocked completed rows" value={blockedRows} /></div><p className="mt-3 text-sm text-amber-200">Current/incomplete rows excluded from this payload: {payload.validation.currentIncompleteRows.toLocaleString()}.</p><button type="button" disabled={Boolean(needsReview.length || !ready || applying)} onClick={() => void apply()} className="mt-4 rounded bg-emerald-700 px-5 py-3 font-black disabled:cursor-not-allowed disabled:bg-zinc-700">{applying ? "Submitting reviewed observations…" : needsReview.length ? `${needsReview.length} name(s) still need review` : !ready ? "Blocked by source validation" : "Commit reviewed Monthly observations"}</button></section>
