...
type Payload = {
  ...
  validation: { totalRows: number; scoreRows: number; noSubmissionRows: number; malformedRows: number; duplicateRows: number; conflictingRows: number; negativeScores: number; totalMismatches: number; completedTotalRows: number; completedScoreRows: number; completedNoSubmissionRows: number; completedMalformedRows: number; currentIncompleteRows: number; currentIncompleteScoreRows: number; currentIncompleteNoSubmissionRows: number; currentIncompleteMalformedRows: number }
  rows: MonthlyWebsiteObservation[]
  periods: { period: string; year: number; month: number; status: "completed" | "current_incomplete"; importable: boolean; reason: string | null; rows: number; scoredRows: number; noSubmissionRows: number; malformedRows: number }[]
...
export default function MonthlyWebsiteImporter() {
  ...
  const [messageTone, setMessageTone] = useState<"success" | "error" | "progress">("success")
  ...
  useEffect(() => {
    if (!message || messageTone === "progress") return
  ...
  const names = useMemo(() => {
    if (!payload) return []
    const counts = new Map<string, { key: string; name: string; count: number; scored: number; noSubmission: number; periods: string[] }>()
    for (const row of payload.rows.filter(row => row.importable)) {
      const key = identityKey(row.historicalPlayerName)
      const current = counts.get(key) ?? { key, name: row.historicalPlayerName, count: 0, scored: 0, noSubmission: 0, periods: [] }
      current.count += 1
      if (row.score !== null) current.scored += 1
      else if (row.scoreState === "NO SUBMISSION") current.noSubmission += 1
  ...
  const ready = Boolean(payload && payload.identityValidation.ready && payload.validation.totalRows === payload.manifest.rawRenderedRows && payload.validation.scoreRows === payload.manifest.scoreObservations && payload.validation.noSubmissionRows === payload.manifest.missingScoreObservations && payload.validation.duplicateRows === 0 && payload.validation.conflictingRows === 0 && payload.validation.totalMismatches === 0 && payload.productionOverlap.available && payload.productionOverlap.trueConflictRows === 0)
  ...
  async function apply() {
    ...
    setApplying(true); setMessageTone("progress"); setMessage("Committing reviewed Monthly observations…")
    try {
      const { data } = await supabase.auth.getSession()
      const token = data.session?.access_token
      if (!token) throw new Error("An authenticated site-admin session is required.")
      ...
      const rpcRows = Array.isArray(result.data) ? result.data[0] as { idempotent?: boolean; applied_row_count?: number } | undefined : undefined
      const outcome = rpcRows?.idempotent ? `already existed; ${rpcRows.applied_row_count?.toLocaleString() ?? rows.length.toLocaleString()} rows are idempotent` : `inserted ${rpcRows?.applied_row_count?.toLocaleString() ?? rows.length.toLocaleString()} rows`
      setMessageTone("success")
      setMessage(`Monthly commit succeeded: ${outcome}. ${payload.validation.noSubmissionRows.toLocaleString()} no-submission slots and ${payload.validation.currentIncompleteRows.toLocaleString()} current/incomplete rows were not imported.`)
    } catch (error) {
  ...
  return <main className="mx-auto max-w-7xl p-6 text-white">
    ...
    <section className="mt-6 rounded-xl border border-zinc-700 bg-zinc-900 p-5"><h2 className="text-xl font-bold">Source validation</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Stat label="Completed periods" value={payload.periods.filter(period => period.importable).length} /><Stat label="Historical rows" value={payload.validation.completedTotalRows} /><Stat label="Actual historical scores ready" value={payload.validation.completedScoreRows} /><Stat label="No submission" value={payload.validation.completedNoSubmissionRows} /><Stat label="Malformed / recovery problems" value={payload.validation.completedMalformedRows} /><Stat label="Exact names" value={names.length} /><Stat label="Negative scores" value={payload.validation.negativeScores} /><Stat label="Duplicate rows" value={payload.validation.duplicateRows} /><Stat label="Conflicts / totals" value={payload.validation.conflictingRows + payload.validation.totalMismatches} /></div><p className="mt-4 break-all font-mono text-xs text-zinc-500">SHA-256 {payload.sourceSha256} · {payload.manifest.coverage.earliest} through {payload.manifest.coverage.latest}</p><p className="mt-3 rounded border border-amber-700 bg-amber-950 p-3 text-amber-100"><strong>Current period blocked:</strong> {payload.manifest.finalization.currentPeriodReason} Rows after {payload.manifest.finalization.finalizedThrough} remain preserved source evidence but are not importable until explicitly finalized.</p><p className="mt-3 text-sm text-zinc-400">{payload.validation.currentIncompleteRows.toLocaleString()} current/incomplete rows excluded ({payload.validation.currentIncompleteScoreRows.toLocaleString()} scored, {payload.validation.currentIncompleteNoSubmissionRows.toLocaleString()} no submission).</p>{payload.existingImport && <p className="mt-3 rounded border border-emerald-800 bg-emerald-950 p-3 text-emerald-200">This exact source SHA already has a Monthly import record with {payload.existingImport.applied_row_count.toLocaleString()} applied observation(s); retrying is idempotent.</p>}</section>
