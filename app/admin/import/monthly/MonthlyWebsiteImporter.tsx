...
import type { MonthlyWebsiteObservation } from "@/lib/importer/adapters/monthlyWebsiteAdapter"

const supabase = createBrowserSupabaseClient()
const PARSER_VERSION = "historical-monthly-website-v1"
...
type Identity = { key: string; historicalName: string; status: "resolved" | "ambiguous" | "unresolved"; playerId: string | null; playerName: string | null; confidence: number; evidence: string }
type Payload = {
  parserVersion: string
  sourceFile: string
  sourceSha256: string
  manifest: { rawRenderedRows: number; scoreObservations: number; missingScoreObservations: number; coverage: { earliest: string; latest: string; periodCount: number; periods: string[] } }
  validation: { totalRows: number; scoreRows: number; missingScoreRows: number; duplicateRows: number; conflictingRows: number; negativeScores: number; totalMismatches: number }
...
export default function MonthlyWebsiteImporter() {
  ...
  const names = useMemo(() => {
    if (!payload) return []
    const counts = new Map<string, { key: string; name: string; count: number; scored: number }>()
    for (const row of payload.rows) {
      const key = identityKey(row.historicalPlayerName)
      const current = counts.get(key) ?? { key, name: row.historicalPlayerName, count: 0, scored: 0 }
      current.count += 1
  ...
  const effective = (key: string): { playerId: string; playerName: string; source: "automatic" | "manual" } | null => {
  ...
  const needsReview = names.filter(person => !effective(person.key))
  const validRows = payload?.rows.filter(row => row.score !== null && row.issues.length === 0 && effective(identityKey(row.historicalPlayerName))) ?? []
  const blockedRows = payload ? payload.rows.length - validRows.length : 0
  const ready = Boolean(payload && payload.validation.totalRows === payload.manifest.rawRenderedRows && payload.validation.scoreRows === payload.manifest.scoreObservations && payload.validation.missingScoreRows === payload.manifest.missingScoreObservations && payload.validation.duplicateRows === 0 && payload.validation.conflictingRows === 0 && payload.validation.totalMismatches === 0)
  const visibleNames = names.filter(person => !needsOnly || !effective(person.key)).sort((left, right) => Number(Boolean(effective(left.key))) - Number(Boolean(effective(right.key))) || left.name.localeCompare(right.name))

  function choose(key: string, playerId: string) {
  ...
  function exportDraft() {
    if (!payload) return
    const draft = { schema: "historical-monthly-website-review-v1", sourceSha256: payload.sourceSha256, decisions, exportedAt: new Date().toISOString() }
  ...
  return <main className="mx-auto max-w-7xl p-6 text-white">
    ...
    <section className="mt-6 rounded-xl border border-zinc-700 bg-zinc-900 p-5"><h2 className="text-xl font-bold">Source validation</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Stat label="Periods" value={payload.manifest.coverage.periodCount} /><Stat label="Rendered rows" value={payload.validation.totalRows} /><Stat label="Scored observations" value={payload.validation.scoreRows} /><Stat label="Missing-score blocked" value={payload.validation.missingScoreRows} /><Stat label="Exact names" value={names.length} /><Stat label="Negative scores" value={payload.validation.negativeScores} /><Stat label="Duplicate rows" value={payload.validation.duplicateRows} /><Stat label="Conflicts / totals" value={payload.validation.conflictingRows + payload.validation.totalMismatches} /></div><p className="mt-4 break-all font-mono text-xs text-zinc-500">SHA-256 {payload.sourceSha256} · {payload.manifest.coverage.earliest} through {payload.manifest.coverage.latest}</p>{payload.existingImport && <p className="mt-3 rounded border border-emerald-800 bg-emerald-950 p-3 text-emerald-200">This exact source SHA already has a Monthly import record with {payload.existingImport.applied_row_count.toLocaleString()} applied observation(s); retrying is idempotent.</p>}</section>
    <section className="mt-6 rounded-xl border border-zinc-700 bg-zinc-900 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">Review {needsReview.length} names requiring a decision</h2><p className="mt-1 text-sm text-zinc-400">Unresolved and ambiguous names stay at the top. One approved selection applies to every occurrence of that exact historical name.</p></div><span className="rounded bg-amber-950 px-3 py-2 font-bold text-amber-200">{needsReview.length} unresolved / ambiguous</span></div><label className="mt-4 inline-flex items-center gap-2"><input type="checkbox" checked={needsOnly} onChange={event => setNeedsOnly(event.target.checked)} /> Needs review only</label><div className="mt-4 space-y-3">{visibleNames.map(person => { const identity = identityFor(person.key); const decision = effective(person.key); const query = queryByName[person.key] ?? ""; const options = payload.players.filter(player => !query || [player.screen_name, ...payload.aliases.filter(alias => alias.player_id === player.id).map(alias => alias.alias)].some(value => value.toLocaleLowerCase().includes(query.toLocaleLowerCase()))).slice(0, 20); return <article key={person.key} className={`rounded-lg border p-4 ${decision ? "border-emerald-800" : "border-amber-500"}`}><div className="flex flex-wrap justify-between gap-2"><div><h3 className="text-lg font-bold">{person.name}</h3><p className="text-sm text-zinc-400">{person.scored.toLocaleString()} scored row(s) · {person.count.toLocaleString()} rendered row(s) · {identity?.status ?? "unresolved"} {identity?.evidence !== "none" ? `· ${identity?.evidence}` : ""}</p></div><span className={decision ? "text-emerald-300" : "text-amber-300"}>{decision ? `✓ ${decision.playerName} · ${decision.source}` : "Needs canonical Global Player"}</span></div>{!decision && <><input value={query} onChange={event => setQueryByName(current => ({ ...current, [person.key]: event.target.value }))} placeholder="Search existing Global Players or verified aliases" className="mt-3 w-full rounded border border-zinc-600 bg-black px-3 py-2" />{query && <div className="mt-2 space-y-2">{options.map(player => <button type="button" key={player.id} onClick={() => choose(person.key, player.id)} className="block w-full rounded border border-zinc-700 p-3 text-left hover:border-indigo-400"><span className="font-bold">{player.screen_name}</span><span className="ml-2 text-xs text-zinc-400">{player.active ? "active" : "inactive/archived"}</span></button>)}{options.length === 0 && <p className="text-sm text-zinc-400">No canonical Global Player found. Do not create or guess one.</p>}</div>}</>}{decision && <button type="button" onClick={() => setDecisions(current => { const next = { ...current }; delete next[person.key]; return next })} className="mt-3 rounded border border-zinc-600 px-3 py-1">Clear manual selection</button>}</article> })}</div></section>
    <section className="mt-6 rounded-xl border border-zinc-700 bg-zinc-900 p-5"><h2 className="text-xl font-bold">Representative score observations</h2><p className="mt-1 text-sm text-zinc-400">The complete preserved CSV remains staged; this compact view avoids rendering thousands of rows at once.</p><div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead><tr className="border-b border-zinc-700 text-zinc-400"><th className="p-2">Year / month</th><th className="p-2">Division</th><th className="p-2">Historical name</th><th className="p-2">Course</th><th className="p-2">Difficulty</th><th className="p-2">Score</th><th className="p-2">Place</th><th className="p-2">Points</th><th className="p-2">Identity</th></tr></thead><tbody>{payload.rows.filter(row => row.score !== null).slice(0, 150).map(row => { const match = effective(identityKey(row.historicalPlayerName)); return <tr key={row.sourceFingerprint} className="border-b border-zinc-800"><td className="p-2">{row.year} / {row.month}</td><td className="p-2">{row.division}</td><td className="p-2">{row.historicalPlayerName}</td><td className="p-2">{row.courseName}</td><td className="p-2">{row.difficulty}</td><td className="p-2 font-bold">{row.score}</td><td className="p-2">{row.coursePlacement ?? "—"}</td><td className="p-2">{row.coursePoints ?? "—"}</td><td className={match ? "p-2 text-emerald-300" : "p-2 text-amber-300"}>{match?.playerName ?? identityFor(identityKey(row.historicalPlayerName))?.status ?? "unresolved"}</td></tr> })}</tbody></table></div></section>
    <section className="mt-6 rounded-xl border border-zinc-700 bg-zinc-900 p-5"><h2 className="text-xl font-bold">Commit reviewed Monthly observations</h2><p className="mt-2 text-zinc-300">Only scored rows with a canonical UUID are sent to the installed RPC. Missing-score rows remain blocked and are never sent.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><Stat label="Resolved names" value={names.length - needsReview.length} /><Stat label="Ready observations" value={validRows.length} /><Stat label="Blocked observations" value={blockedRows} /></div><button type="button" disabled={Boolean(needsReview.length || !ready || applying)} onClick={() => void apply()} className="mt-4 rounded bg-emerald-700 px-5 py-3 font-black disabled:cursor-not-allowed disabled:bg-zinc-700">{applying ? "Submitting reviewed observations…" : needsReview.length ? `${needsReview.length} name(s) still need review` : !ready ? "Blocked by source validation" : "Commit reviewed Monthly observations"}</button></section>
