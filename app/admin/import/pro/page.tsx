...
export default function HistoricalProImporterPage() {
  ...
  const partialPairings = (preview?.seasonPairings ?? []).filter((pairing) => pairing.evidenceType === "SOURCE COLOR CONFIRMED" && pairing.gameState === "PARTIAL / INCOMPLETE")
  const proxyPairings = (preview?.seasonPairings ?? []).filter((pairing) => pairing.evidenceType === "SOURCE COLOR CONFIRMED" && pairing.gameState === "PROXY ROUND — OPPONENT DID NOT PLAY")
