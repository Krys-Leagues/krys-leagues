...
export default function HistoricalProImporterPage() {
  ...
  const partialPairings = (preview?.seasonPairings ?? []).filter((pairing) => pairing.evidenceType === "SOURCE COLOR CONFIRMED" && pairing.pairingState === "PARTIAL — NEEDS REVIEW")
  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-8">
    ...
    <section className="rounded-xl border border-zinc-700 bg-zinc-950 p-5"><h2 className="text-2xl font-bold">Season 1–12 identity-review readiness</h2><div className="mt-3 grid gap-2 sm:grid-cols-3"><div>Resolved score rows: <strong>{readyRows.length}</strong></div><div>Identity blockers: <strong className={blockers.length ? "text-amber-300" : "text-emerald-300"}>{blockers.length}</strong></div><div>Partial pairing reviews: <strong className="text-amber-300">{preview.pairingSummary.partialScoreReview}</strong></div></div>{blockers.length > 0 && <p className="mt-3 text-amber-200">Review every unresolved/ambiguous name attached to an eligible Season 1–12 score row before any future commit. This page does not commit scores or change identities automatically.</p>}{unreviewedPartialPairings.length > 0 && <p className="mt-3 text-amber-200">Commit remains disabled while {unreviewedPartialPairings.length} partial pairing review(s) remain unresolved. Items marked UNKNOWN / NEEDS LATER REVIEW remain excluded from any future import payload.</p>}<button type="button" disabled className="mt-4 rounded bg-zinc-700 px-5 py-3 font-bold text-zinc-400">Commit Historical Pro scores (review-only phase)</button></section>
  </main>
}
