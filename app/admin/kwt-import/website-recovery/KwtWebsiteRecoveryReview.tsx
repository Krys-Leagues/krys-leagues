...
export default function KwtWebsiteRecoveryReview() {
  ...
  async function apply() {
    ...
    try {
      ...
      for (const source of payload.sources) {
        ...
        appliedRows += sourceRows.length
      }
      setMessage(`Applied ${appliedRows.toLocaleString()} valid player-weeks. ${malformedRows} malformed row(s) remain blocked.`)
    } catch (error) {
  ...
  return <main className="mx-auto max-w-7xl p-6 text-white">
    ...
    <section className="mt-6 rounded-xl border border-zinc-700 bg-zinc-900 p-5"><h2 className="text-xl font-bold">Recovery and safety summary</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Stat label="Seasons" value={new Set(payload.sources.map((source) => source.season)).size} /><Stat label="Weeks" value={periods} /><Stat label="Player-weeks" value={rows.length} /><Stat label="Easy observations" value={rows.length} /><Stat label="Hard observations" value={rows.length - malformedRows} /><Stat label="Unique names" value={names.length} /><Stat label="Automatic exact" value={automaticCount} /><Stat label="Needs review" value={unresolvedNames.length} /><Stat label="Malformed blocked" value={malformedRows} /><Stat label="Existing source files" value={existingSourceCount} /></div><p className="mt-4 text-sm text-zinc-400">Valid source rows: {rows.length.toLocaleString()} · Valid total observations: {rows.length.toLocaleString()} · Duplicate rows: {payload.sources.reduce((sum, source) => sum + source.duplicateRows, 0)} · Conflicting observations: {hardBlockers.length ? "blocked for review" : "0"} · Existing scorecards: {payload.existingScorecardCount === null ? "unavailable" : payload.existingScorecardCount.toLocaleString()}</p><p className="mt-2 text-xs text-zinc-500">Source SHA values are the preserved raw KWT result-page SHA-256 values. Historical rank/division remains null when the source does not supply it; current website rank is never inferred.</p>{payload.databaseReadError && <p className="mt-2 text-amber-300">Database read note: {payload.databaseReadError}</p>}</section>
