...
export default function MatchSetup() {
  ...
  async function loadSetupData(
    ...
    if (!requestedSeasonId) {
      setSetupError("A seasonId is required to load Match setup.")
      setLoadingSetup(false)
      return
    }
    ...
    const membersResponse = await fetch(`/api/admin/league-memberships?league_type=match&season_number=${encodeURIComponent(String(selectedSeason.season_number))}&division=${encodeURIComponent(`Match D${requestedDivision}`)}`, { cache: "no-store", credentials: "same-origin" })
    ...
    setDivision(`Match D${requestedDivision}`)
  ...
  function navigateToDivision(nextDivision: number, ignoreRosterChanges = false) {
    ...
    router.push(`/admin/match/setup?${params.toString()}`)
  ...
  async function saveDivisionRoster() {
    ...
    if (hasNextDivision) {
      ...
      if (navigateToDivision(nextDivision, true)) {
        setRosterMessage(
          `Match D${savedDivision} roster saved. Match D${nextDivision} is ready.`
        )
      } else {
        setRosterMessage(
          `Match D${savedDivision} roster saved. Unsaved course changes remain on this division.`
        )
      }
    } else if (rosterStatus === "draft") {
      setRosterMessage(
        `Match D${divisionNumber} roster saved. This is the final division.`
      )
    } else {
      await loadScheduleState(seasonId)
      setRosterMessage(`Match D${divisionNumber} roster saved.`)
    }
  ...
  async function saveCourseOverrides() {
    ...
    setCourseMessage(`Match D${divisionNumber} division courses were saved.`)
    setCourseSaveError(false)
    setSavingCourses(false)
  }
  ...
  return (
    <main style={page}>
      <div style={container}>
        <div style={topBar}>
          <button
            onClick={() => navigateAway("/admin/match")}
        ...
        <h1 style={{ fontSize: 36 }}>Match Setup</h1>

        {loadingSetup && <p style={helperText}>Loading Match setup...</p>}
        ...
        <div style={activeDivisionHeader}>
          ...
          <div style={divisionNavigation}>
            ...
            <button
              ...
              onClick={() =>
                navigateAway(
                  `/admin/match/season/edit?seasonId=${encodeURIComponent(seasonId)}`
                )
              }
              style={backButtonSecondary}
            >
            ...
            <button
              ...
              onClick={() =>
                navigateAway(
                  `/admin/match/schedule?seasonId=${encodeURIComponent(seasonId)}`
                )
              }
              style={backButtonPrimary}
            >
              ...
              <div style={workflowActions}>
                ...
                {scheduleState && scheduleState.generated_revision > 0 && (
                  <button
                    ...
                    onClick={() =>
                      navigateAway(
                        `/admin/match/schedule?seasonId=${encodeURIComponent(
                          seasonId
                        )}`
                      )
                    }
