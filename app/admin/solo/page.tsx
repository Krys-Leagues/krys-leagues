...
export default function SoloAdminPage() {
  ...
  return <main style={page}><div style={container}><h1 style={title}>Solo Admin Hub</h1><p style={subtitle}>Managed Solo seasons, flexible division rosters, and four-week setup.</p>
    ...
    <div style={grid}><Link href={selectedId ? href("/admin/solo/season") : "/admin/solo/season"} style={card}><strong>Season</strong><span>Create a season or review the exact selected season.</span></Link>{ready ? <><Link href={href("/admin/solo/setup")} style={card}><strong>Setup / Roster</strong><span>Assign canonical players to Solo divisions.</span></Link><Link href={href("/admin/solo/weeks")} style={card}><strong>Weeks</strong><span>Set the four courses and due dates.</span></Link></> : <><Disabled title="Setup / Roster" text="Select a managed Solo season first."/><Disabled title="Weeks" text="Select a managed Solo season first."/></>}{["Results · Coming next", "Standings · Coming next", "Final Scorecard · Coming later", "Transition · Coming later"].map((x) => <Disabled key={x} title={x} text="This foundation intentionally does not enable this workflow yet."/>)}</div>
  </div></main>
}
