...
export async function GET() {
  ...
  try {
    ...
    const { data: rows, error } = await service.from("course_challenge_submissions").select("id,player_id,course_slug,challenge_key,level_number,difficulty,proof_photo_path,hole_scores,total_par,calculated_total,relative_to_par,requirements_evaluation,status,review_reason,round_date,round_time").in("status", ["pending", "needs_review"]).order("created_at", { ascending: true })
