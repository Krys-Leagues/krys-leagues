...
function strokeAdminClient() {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SECRET_KEY
  if (!url || !key) throw new Error("Stroke admin server access is not configured.")
...
export async function GET(request: Request) {
  ...
  try {
    const supabase = strokeAdminClient()
    ...
    return json(
      { error: error instanceof Error ? error.message : "Stroke course overrides could not be loaded." },
      503
    )
  }
}
