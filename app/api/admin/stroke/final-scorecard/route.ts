...
function adminClient() {
  ...
  if (!url || !key) throw new Error("Stroke final-scorecard server access is not configured.")
  ...
  try {
    ...
    if (scorecardId) {
      ...
      const [entries, decisions] = await Promise.all([
        ...
        supabase.from("stroke_final_scorecard_player_decisions")
    ...
    const [entries, fixtures] = await Promise.all([
      ...
      supabase.from("schedule").select("id").eq("league_type", "stroke").eq("season_id", seasonId)
    ...
    if (fixtureIds.length > 0) {
      const results = await supabase.from("results").select("schedule_id, player1_score, player2_score")
        .eq("league_type", "stroke").in("schedule_id", fixtureIds)
      if (results.error) return json({ error: results.error.message }, 503)
      completedFixtures = (results.data || []).filter((result) => result.player1_score !== null && result.player2_score !== null).length
    }
    return json({ data: { scorecard: selected, entries: entries.data || [], completedFixtures, totalFixtures: fixtureIds.length } })
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : "Stroke final scorecard could not be loaded." }, 503)
  }
}
