...
export async function GET(request: Request) {
  ...
  try {
    ...
    const definitions = new Map([...course.levels.flatMap((level) => levelRewardDefinitions(course, level.level)), ...aceStageRewardDefinitions(course), ...(course.prestigeStages || []).flatMap((stage) => prestigeStageRewardDefinitions(course, stage.stage))].filter(Boolean).map((reward) => [reward!.rewardKey, reward!]))
    ...
    const rewardGroups = [
      { key: "course-pro", label: "Course Pro", kind: "advanced" as const, matches: (row: RewardRow) => row.reward_key.endsWith(":course-pro") },
      { key: "course-master", label: "Course Master", kind: "advanced" as const, matches: (row: RewardRow) => row.reward_key.endsWith(":course-master") },
      ...(course.aceStages || []).map((stage) => ({ key: stage.key, label: stage.label, kind: "ace" as const, matches: (row: RewardRow) => row.reward_key === stage.rewardKey })),
    ]
    const specialGroups = rewardGroups.map((group) => {
      const matchingRows = rewardRows.filter(group.matches)
      return { key: group.key, label: group.label, kind: group.kind, players: [...new Set(matchingRows.map((row) => row.player_id))].map((id) => ({ player: player(id), reward: definitions.get(matchingRows.find((row) => row.player_id === id)?.reward_key || "") })).filter((entry) => entry.player) }
    })
