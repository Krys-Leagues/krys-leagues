...
import { evaluateCourseChallengeRequirements, validHoleScores, validHolePars } from "@/lib/courseChallenges/evaluation"
...
export async function POST(request: Request) {
  try {
    ...
    if (!isDate(body.roundDate) || !isTime(body.roundTime)) return Response.json({ error: "Round date and time are required." }, { status: 400 })
    if (body.gameMode !== "solo" && body.gameMode !== "multiplayer") return Response.json({ error: "Only real Solo or Multiplayer Game Mode rounds qualify. Practice Mode does not qualify." }, { status: 400 })
    const audience = audienceForCanonicalPlayer(identity.playerId, identity.approvedTester)
    const eligibility = eligibleRoundDate(body.roundDate, audience)
    ...
    const status = evaluation.status === "auto_fail" ? "rejected" : "needs_review"
    const insert = await service.from("course_challenge_submissions").insert({ player_id: identity.playerId, course_slug: course.slug, challenge_key: challengeKey, level_number: level, difficulty, proof_photo_path: body.proofPhotoPath, round_date: body.roundDate, round_time: body.roundTime, game_mode: body.gameMode, hole_scores: body.scores, calculated_total: evaluation.metrics.totalStrokes, total_par: evaluation.metrics.totalPar, relative_to_par: evaluation.metrics.relativeToPar, metrics: evaluation.metrics, requirements_evaluation: evaluation.requirements, auto_evaluation_status: evaluation.status, photo_total_check: "needs_review", status, review_reason: evaluation.reason || "The evidence photo and challenge requirements require review." }).select("id,status").single()
    if (insert.error) throw insert.error
    return Response.json({ id: insert.data?.id, status, message: status === "rejected" ? "This card did not meet the currently approved automatic checks." : "Scorecard received. It remains pending review until the evidence and challenge requirements are verified." }, { status: 201 })
