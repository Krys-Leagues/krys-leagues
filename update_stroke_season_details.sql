create or replace function public.update_stroke_season_details(
  p_season_id uuid,
  p_start_date date,
  p_end_date date,
  p_game1_course text,
...
begin
  ...
  if not found then
    if exists (
      ...
      raise exception 'A locked historical Stroke season cannot be changed'
        using errcode = '42501';
    end if;

    raise exception 'No editable draft or approved Stroke roster was found for this season';
  end if;
...
revoke all on function public.update_stroke_season_details(uuid, date, date, text, text, text) from public;
revoke all on function public.update_stroke_season_details(uuid, date, date, text, text, text) from anon;
revoke all on function public.update_stroke_season_details(uuid, date, date, text, text, text) from authenticated;
grant execute on function public.update_stroke_season_details(uuid, date, date, text, text, text) to authenticated;
