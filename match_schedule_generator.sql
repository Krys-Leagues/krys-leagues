create or replace function public.generate_match_schedule(
  p_season_id uuid
)
returns table(
  season_id uuid,
...
begin
  ...
  if v_user_id is null then
    raise exception
      'Authentication is required to generate a Match schedule'
      using errcode = '42501';
  end if;
  ...
  -- Lock the approved roster first. This follows the shared Match lock
  ...
  if not found then
    if exists (
      ...
      raise exception
        'A locked historical Match roster cannot have its schedule regenerated'
        using errcode = '42501';
    end if;

    raise exception
      'Exactly one approved Match roster is required before schedule generation';
  end if;
  ...
  if lower(btrim(v_season.league_type)) <> 'match' then
    raise exception 'The requested season is not a Match season';
  end if;
  ...
  if v_roster.division_count is null or v_roster.division_count <= 0 then
    raise exception
      'The approved Match roster must have a positive division count';
  end if;
  ...
  if not found then
    raise exception
      'The approved Match roster has no schedule workflow state';
  end if;
  ...
  if exists (
    ...
    left join public.match_division_roster_slots as roster_slot
  ...
  if v_generated_revision = v_change_revision then
    ...
    where lower(btrim(scheduled_fixture.league_type)) = 'match'
    ...
    and scheduled_fixture.match_roster_version_id = v_roster.id
      and exists (
        ...
        where completed_result.schedule_id = scheduled_fixture.id
          and lower(btrim(completed_result.league_type)) = 'match'
  ...
  desired_rows as (
    ...
    left join public.match_division_course_overrides as course_override
  ...
  -- Lock only managed Match fixtures for this season. Legacy/test rows
  ...
  if exists (
    ...
    raise exception
      'A completed Match fixture conflicts with the current approved roster; completed history was not changed';
  end if;
  ...
  if exists (
    ...
    raise exception
      'An obsolete Match fixture has a linked result and cannot be removed safely';
  end if;
  ...
  set
    ...
    division = 'Match D' || desired_row.division_number::text,
    ...
    match_roster_version_id = v_roster.id,
    division_number = desired_row.division_number,
    game_number = desired_row.game_number
  from desired_rows as desired_row
  where lower(btrim(scheduled_fixture.league_type)) = 'match'
    ...
    and row(
      ...
      scheduled_fixture.match_roster_version_id,
      scheduled_fixture.division_number,
      scheduled_fixture.game_number
    ) is distinct from row(
      'match'::text,
      v_season.season_number,
      'Match D' || desired_row.division_number::text,
  ...
  where not exists (
    ...
    where lower(btrim(existing_fixture.league_type)) = 'match'
      and existing_fixture.season_id = p_season_id
      and existing_fixture.match_roster_version_id = v_roster.id
...
revoke all
  on function public.generate_match_schedule(uuid)
  from public;
